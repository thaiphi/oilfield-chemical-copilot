...
| 2. Fresh E1a-4 population contracts and private population | `IN_PROGRESS` | The authenticated role mapping and deterministic metadata-only sampling frame are sealed and independently no-write verified. The frame has 96 slots, 96 unique slot identities, and 96 unique source-locator assignments across all four topics and both roles, with zero E1a-3 locator reuse. The next checkpoint is private question and canonical-claim authoring; retrieval, ingestion, and reindexing are not authorized. |
...
- [x] Seal the metadata-only E1a-4 source register and exact 96-slot allocation after database and index prerequisites pass.
  ...
  - **Closed 2026-08-28 after approved application:** the authenticated mapping and frame were sealed and independently no-write verified. The frame contains exactly 96 slots with unique slot identities and source-locator assignments, covers all four topics and both roles, and has zero E1a-3 locator reuse. The execution boundary remains unchanged: question and canonical-claim authoring is the next checkpoint; do not retrieve, ingest, or reindex.
